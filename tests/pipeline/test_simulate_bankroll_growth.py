...
# Mock load_config to avoid file dependency
@patch("tennis_betting_model.pipeline.simulate_bankroll_growth.load_config")
def test_simulation_flat_strategy(mock_load_config, sample_bets):
    """Tests the 'flat' staking strategy."""
    mock_load_config.return_value = {"simulation_params": {}}
    initial_bankroll = 100.0
    stake_unit = 10.0  # Bet 10 units each time

    result_df = simulate_bankroll_growth(
        sample_bets, initial_bankroll, strategy="flat", stake_unit=stake_unit
    )
...
@patch("tennis_betting_model.pipeline.simulate_bankroll_growth.load_config")
def test_simulation_kelly_strategy(mock_load_config, sample_bets):
    ...
    result_df = simulate_bankroll_growth(
        sample_bets, initial_bankroll, strategy="kelly", kelly_fraction=kelly_fraction
    )
