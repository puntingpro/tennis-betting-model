...
from src.scripts.utils.consolidate_data import consolidate_data
...
def test_consolidate_data_filters_unwanted_files(tmp_path: Path):
    ...
    (data_dir / "atp_matches_2023.csv").write_text("tourney_date,winner_id\n20230105,101")
    (data_dir / "atp_matches_2022.csv").write_text("tourney_date,winner_id\n20221225,102")

    # Create files that should be EXCLUDED by the filter logic
    (data_dir / "atp_matches_qual_2023.csv").write_text("tourney_date,winner_id\n20230101,901")
    (data_dir / "atp_matches_doubles_2023.csv").write_text("tourney_date,winner_id\n20230102,902")
    (data_dir / "atp_matches_futures_2023.csv").write_text("tourney_date,winner_id\n20230103,903")
