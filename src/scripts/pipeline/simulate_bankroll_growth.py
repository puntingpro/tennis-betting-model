...
def simulate_bankroll_growth(
    ...
    if strategy == "flat":
        ...
        df["stake"] = df["bankroll_start"].shift(1).fillna(initial_bankroll) * (stake_unit / 100)
    elif strategy == "kelly":
        ...
        df["stake"] = df["bankroll_start"].shift(1).fillna(initial_bankroll) * limited_kelly
    else:
    ...
    df["bankroll_end"] = df["bankroll_start"].shift(1).fillna(initial_bankroll) + df["profit"]
    
    # Drop the temporary start column for a cleaner output
    df = df.drop(columns=["bankroll_start"]).rename(columns={"bankroll_end": "bankroll"})
